...
namespace proc
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            //AppDomain app = AppDomain.CreateDomain("Pasha");
            //Console.WriteLine($"\nДомен: {app.FriendlyName}");
            //app.ExecuteAssembly("E:\\2 курс\\2_course\\ООП\\Labs_2\\ConsoleApp1\\bin\\Debug\\netcoreapp3.0\\ConsoleApp1.exe");
            ...
            Console.WriteLine("Введите число для вывода чётных и нечётных чисел до него");
            int number2 = int.Parse(Console.ReadLine());

            // Console.WriteLine("Введите число для вывода нечётных чисел до него");
            //int number3 = int.Parse(Console.ReadLine());
            TimerCallback timer = new TimerCallback(TimerWork);
            Timer timer2 = new Timer(timer, null, 0, 100);
            ...
            Thread thread4 = new Thread(new ParameterizedThreadStart(Even));
            thread4.Name = "EvenNumberThread OddNumberThread";
            Console.WriteLine($"Поток:{thread4.Name} Приоритет:{thread4.Priority}");
            thread4.Start(number2);



        }
        public static void TimerWork(object NoObject)
        {
            Console.WriteLine("Таймер работает");

        }

        public static void YourNumbers(object n)
        {
            string writePath = "E:\\лаба15\\numbers.txt";
        ...
        {
            string writePath = "E:\\лаба15\\AllNumbers.txt";
            Thread t = Thread.CurrentThread;
            lock(locker)
        {
                for (int i = 1; i <= (int)n; i++)
        ...
        {
            string writePath = "E:\\лаба15\\Numbers.txt";
            Thread t = Thread.CurrentThread;
            lock (locker)
            {
                for (int i = 1; i <= (int)n; i++)
                {
                    if (t.Name.Contains("EvenNumberThread"))
                    {
                        if (i % 2 == 0)
                        {
                            Console.WriteLine(i);
                    ...
                    if (t.Name.Contains("OddNumberThread"))
                    {
                        if (i % 2 != 0)
                        {
                            Console.WriteLine(i);
